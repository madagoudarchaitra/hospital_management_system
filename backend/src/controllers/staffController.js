...
exports.list = async (req, res) => {
  try{ const list = await Staff.findAll(); res.json(list); }catch(err){ res.status(500).json({ message: 'Server error' }); }
};

exports.get = async (req, res) => {
  try{ const s = await Staff.findByPk(req.params.id); if(!s) return res.status(404).json({}); res.json(s); }catch(err){ res.status(500).json({ message: 'Server error' }); }
};
