...
exports.list = async (req, res) => {
  try{ const list = await LabReport.findAll(); res.json(list); }catch(err){ res.status(500).json({ message: 'Server error' }); }
};

exports.get = async (req, res) => {
  try{ const r = await LabReport.findByPk(req.params.id); if(!r) return res.status(404).json({}); res.json(r); }catch(err){ res.status(500).json({ message: 'Server error' }); }
};
