...
exports.list = async (req, res) => {
  try{ const list = await Billing.findAll(); res.json(list); }catch(err){ res.status(500).json({ message: 'Server error' }); }
};

exports.get = async (req, res) => {
  try{ const b = await Billing.findByPk(req.params.id); if(!b) return res.status(404).json({}); res.json(b); }catch(err){ res.status(500).json({ message: 'Server error' }); }
};
